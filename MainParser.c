...
i8 errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern i8 syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern i8 line; /* source code line number - defined in scanner.c */
...
extern ggvoid startParser(mold_void);
extern i8 startScanner(ReaderPointer sc_buf);

static ggvoid printParserError(string fmt, ...);
static ggvoid displayParser(ReaderPointer ptrBuffer);
static gglong getParserFileSize(string fname);
static ggvoid callGarbageCollector(mold_void);
...
i8 mainParser(i8 argc, string* argv) {

	numParserErrors = 0;			/* Initializes the errors */

	FILE* fi;       /* input file handle */
	i8 loadsize = 0; /*the size of the file loaded in the buffer */
...
ggvoid printParserError(string fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(ggvoid)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
gglong getParserFileSize(string fname) {
	FILE* input;
	gglong flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
ggvoid displayParser(ReaderPointer ptrBuffer) {
...
ggvoid callGarbageCollector(mold_void) {
