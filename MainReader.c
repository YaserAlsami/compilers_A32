 ...
 * -------------------------------------------------------------
 */
ggvoid bErrorPrint(ggchar* fmt, ...);
ggvoid displayBuffer(BufferReader* ptr_Buffer);
f64 getFileSize(ggchar* fname);
i8 isNumber(const ggchar* ns);
ggvoid startReader(ggchar*, ggchar*, ggchar, i8, i8);
...
************************************************************
*/
i8 mainReader(int argc, char** argv) {

	/* Create source input buffer */
	string program = argv[0];
	string input = argv[2];
	ggchar mode = MODE_FIXED;
	i8 size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
ggvoid startReader(ggchar* program, ggchar* input, ggchar mode, i8 size, i8 increment) {

	ReaderPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	i8 loadSize = 0;		/* the size of the file loaded in the buffer */
	ggchar symbol;			/* symbol read from input file */
