...
/* External objects */
extern i8 line; /* Source code line numbers - defined in scanner.c */
extern Token tokenizer(ggvoid);
 ...
 * -------------------------------------------------------------
 */
ggvoid printScannerError(ggchar* fmt, ...);
ggvoid displayScanner(BufferReader* ptrBuffer);
gglong getScannerFilesize(string fname);
...
i8 mainScanner(i8 argc, string* argv) {
	...
	printf("\n----------------------------------\n");
	printf("Number of scanner errors: %d",numScannerErrors);
	printf("\n----------------------------------\n");
